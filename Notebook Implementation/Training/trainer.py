...
def WGAN_trainer(opt):
    ...
    # ----------------------------------------
    #  Load Pretrained Generator (if provided)
    # ----------------------------------------
    if opt.pretrained_model_path != '' and os.path.exists(opt.pretrained_model_path):
        print("Loading pretrained generator weights from:", opt.pretrained_model_path)
        pretrained_dict = torch.load(opt.pretrained_model_path)
        if hasattr(generator, 'module'):
            generator.module.load_state_dict(pretrained_dict, strict=False)
        else:
            generator.load_state_dict(pretrained_dict, strict=False)
